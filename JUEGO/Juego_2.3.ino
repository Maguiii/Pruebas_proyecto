 ...
 * ESTA VERSION
 * 
 * Todo lo que se encontraba en el else, se llevo a una funcion llamada juego(), la funcion qeu llevaba ese nombre ahora es
 * encendidoLeds, el objetivo es acortar el codigo para cuando haya que unirlo con los servos y el bluetooth
 * 
...
void loop() {
  ...
  if(aceptacion == 0){
    /*Todas las variables se vuelven a iniciar para poder iniciar una segunda ronda del juego*/
    numViajes = 0; 
    contadorViajes = 0;
    flagRegresion = 0;
    aux = 0; 
...
void finDelJuego(){ 
   ...
   *  
   *  Al finalizar todas estas muestras aceptacion tomara el valor 0 para volver a iniciar el juego en el caso asi se desee
  */
  lcd.clear();
  lcd.setCursor(0,0);
